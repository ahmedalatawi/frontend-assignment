  ...
  return (
    <li className="[&:not(:first-child)]:pt-4">
      <div className="flex items-center space-x-3 rtl:space-x-reverse rounded-xl border bg-dark border-borderColor p-3">
        ...
        <p className="sm:text-base text-xs font-medium text-white">{name}</p>
        ...
        <div className="flex items-center gap-3 sm:text-sm text-xs text-white">
          <span className="font-normal">${price}</span>
          <button
            type="button"
            onClick={() => onSelect(product)}
            className="py-2 px-4 border border-white text-nowrap font-medium cursor-pointer rounded-lg hover:opacity-75"
          >
            View Details
          </button>
        </div>
